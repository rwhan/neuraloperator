...
class RNO_cell(nn.Module):
    ...
    def forward(self, x, h):
        ...
        ----------
        x : torch.Tensor
            Input function at current timestep with shape (batch, width, *spatial_dims)
        h : torch.Tensor
            Hidden state from previous timestep with shape (batch, width, *spatial_dims_h)
            where spatial_dims_h may differ from spatial_dims if resolution_scaling_factor is set

        Returns
        -------
        torch.Tensor
            Updated hidden state with shape (batch, width, *spatial_dims_h)
        """
...
class RNOBlock(nn.Module):
    ...
    def forward(self, x, h=None):
        ...
        ----------
        x : torch.Tensor
            Input sequence with shape (batch, timesteps, width, *spatial_dims)
        h : torch.Tensor, optional
            Initial hidden state with shape (batch, width, *spatial_dims_h).
        ...
        torch.Tensor
            If return_sequences=True: hidden states for all timesteps with shape
                (batch, timesteps, width, *spatial_dims_h)
            If return_sequences=False: final hidden state with shape
                (batch, width, *spatial_dims_h)
        """
        ...
        if h is None:
            ...
            if not self.resolution_scaling_factor:
                h_shape = (batch_size, self.width, *dom_sizes)
            else:
                scaled_sizes = tuple([int(round(self.resolution_scaling_factor*s)) for s in dom_sizes])
                h_shape = (batch_size, self.width, *scaled_sizes)
